...
app.addContentTypeParser('application/x-ndjson', { parseAs: 'string', bodyLimit: 50 * 1024 * 1024 }, (_req, body, done) => {
  try {
    const rows: unknown[] = [];
...
// Passthrough parser for streaming restore: give the handler the raw payload stream as `request.body`.
// The handler iterates the stream line-by-line without ever buffering — required for multi-GB backups.
app.addContentTypeParser('application/x-ndjson-stream', (_req: any, payload: any, done: any) => {
  done(null, payload);
});
...
const { timescaleRoutes } = await import('./modules/timescale/timescale.routes.js');
await app.register(timescaleRoutes, { prefix: '/api/projects' });
const { txnRoutes } = await import('./modules/ai-gateway/txn.routes.js');
await app.register(txnRoutes, { prefix: '/api/projects' });
...
// pg_notify → WebSocket bridge. LISTEN on channel "df_change"; df_emit(...) from any SQL
// (custom_sql endpoints, execute_sql_mutation, cron, triggers, direct sessions) flows here.
// Register the onClose hook BEFORE app.listen() — Fastify rejects addHook once listening.
const { initPgNotifyListener } = await import('./modules/realtime/pg-listener.service.js');
const pgNotifyListener = initPgNotifyListener(app.db, app.log);
app.addHook('onClose', async () => { await pgNotifyListener.stop(); });
...
try {
  ...
  pgNotifyListener.start().catch((err) => app.log.error({ err }, 'Failed to start pg_notify listener'));
