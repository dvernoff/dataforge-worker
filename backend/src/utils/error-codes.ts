...
export const ERROR_CODES: ErrorCodeDef[] = [
  ...
  },
  {
    ...
    cause: 'The query references a schema, table or alias that is not defined in this statement and is not your project schema.',
    suggestion: 'Allowed `X.Y` prefixes: (1) your project schema, (2) a table name that appears in FROM/INTO/UPDATE/JOIN in this same statement, (3) an alias you defined via `AS alias`, (4) a CTE defined via `WITH cte AS (...)`, (5) the built-in pseudo-relations EXCLUDED/OLD/NEW. For ON CONFLICT DO UPDATE use EXCLUDED.col to reference the incoming row and players.col (or your alias) to reference the existing row.',
  },
  {
    code: 'DF_SCHEMA_SHADOW',
    pattern: /identifier "([^"]+)" collides with a database schema name/,
    cause: 'An alias or CTE name in the query matches the name of another database schema. This would allow cross-schema access via alias-shadowing.',
    suggestion: 'Rename the alias to something that does not match any schema name. Example: "INSERT INTO players AS p" instead of "INSERT INTO players AS other_project".',
  },
  {
