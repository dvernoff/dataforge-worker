...
export class SchemaService {
      ...
      switch (change.action) {
        ...
        case 'alter': {
          if (change.type) {
            ...
            const usingClause = NEEDS_USING.has(change.type) ? ` USING "${change.name}"::${pgType}` : '';
            sql = `ALTER TABLE "${schema}"."${tableName}" ALTER COLUMN "${change.name}" TYPE ${pgType}${usingClause}`;
            sqls.push(sql);
          ...
          if (change.default_value !== undefined) {
            if (change.default_value === null) {
              ...
              sql = `ALTER TABLE "${schema}"."${tableName}" ALTER COLUMN "${change.name}" SET DEFAULT ${formatDefaultValue(String(change.default_value))}`;
            }
            sqls.push(sql);
            await this.execAlterSql(sql, change.name);
          }
  ...
  async createMaterializedView(schema: string, input: {
    ...
    validateSchemaAccess(input.query, schema);
