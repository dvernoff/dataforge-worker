...
export class SchemaService {
    ...
    try {
      ...
      if (tsExt) {
        ...
        if (res.rows.length > 0) {
          // Enrich hypertable info with: chunks (capped at 100, with size + compression),
          // retention/compression policy summaries from timescaledb_information.jobs,
          // and oldest/newest chunk range. Each subquery is best-effort — failures fall
          // back to nulls rather than failing the whole describe call.
          const [chunks, retention, compression, range] = await Promise.all([
            this.db.raw(`
              SELECT
            ...
            this.db.raw(`
              SELECT j.job_id, j.config, j.schedule_interval::text AS schedule_interval,
              ...
              WHERE j.proc_name = 'policy_retention'
                AND j.hypertable_schema = ? AND j.hypertable_name = ?
              LIMIT 1
            `, [schema, tableName]).catch(() => ({ rows: [] })),
            this.db.raw(`
              SELECT j.job_id, j.config, j.schedule_interval::text AS schedule_interval,
            ...
            this.db.raw(`
              SELECT
                MIN(range_start::text) AS oldest_chunk_range_start,
                MAX(range_end::text) AS newest_chunk_range_end
              FROM timescaledb_information.chunks
              WHERE hypertable_schema = ? AND hypertable_name = ?
            `, [schema, tableName]).catch(() => ({ rows: [{}] })),
          ]);

          const policyRow = (r: any) => {
            const row = r.rows?.[0];
            if (!row) return null;
            ...
            return {
              ...
              last_run_at: row.last_run_started_at,
              last_run_status: row.last_run_status,
              next_run_at: row.next_start,
            };
          };

          const chunksList = chunks.rows.map((c: Record<string, unknown>) => ({
            name: c.chunk_name,
          ...
          hypertableInfo = {
            ...
            oldest_chunk_range_start: range.rows?.[0]?.oldest_chunk_range_start ?? null,
            newest_chunk_range_end: range.rows?.[0]?.newest_chunk_range_end ?? null,
            retention_policy: policyRow(retention),
            compression_policy: policyRow(compression),
            chunks_shown: chunksList.length,
            chunks: chunksList,
          };
        }
      }
  ...
  async createTable(schema: string, def: TableDefinition): Promise<string> {
    ...
    // Opt-in btree indexes on timestamp columns. Default off to avoid write overhead on tables
    // that don't need them; opt in via index_created_at / index_updated_at when ORDER BY / range
    // queries on these columns are expected.
    if (addCreatedAt && def.index_created_at === true) {
      await this.db.raw(`CREATE INDEX IF NOT EXISTS "idx_${def.name}_created_at" ON "${schema}"."${def.name}" ("created_at" DESC)`).catch(() => {});
    }
    if (addUpdatedAt && def.index_updated_at === true) {
      await this.db.raw(`CREATE INDEX IF NOT EXISTS "idx_${def.name}_updated_at" ON "${schema}"."${def.name}" ("updated_at" DESC)`).catch(() => {});
    }
  ...
  /**
   * List all user-defined triggers on a table. Internal system triggers (FK constraint enforcement,
   * materialized view refresh, etc.) are hidden — only those a user can sensibly enable/disable.
   * For each trigger returns its name, enabled state and a human-readable fire condition.
   */
  async listTriggers(schema: string, tableName: string) {
    validateSchema(schema);
    validateIdentifier(tableName, 'table name');
    const r: any = await this.db.raw(`
      SELECT t.tgname AS name,
             -- tgenabled: 'O' = enabled (ORIGIN), 'D' = disabled, 'R' = replica only, 'A' = always
             ...
             CASE
               ...
               ELSE 'UNKNOWN'
             END AS event,
             CASE t.tgtype::integer & 1 WHEN 1 THEN 'ROW' ELSE 'STATEMENT' END AS granularity,
      ...
      ORDER BY t.tgname
    `, [schema, tableName]);
    return (r.rows as Array<{ name: string; enabled_state: string; timing: string; event: string; granularity: string; function_name: string }>)
      .map(row => ({
        name: row.name,
        enabled: row.enabled_state === 'O' || row.enabled_state === 'A',
        timing: row.timing,
        event: row.event,
  ...
  /**
   * Enable or disable a trigger on a table. Common use case: temporarily disable the
   * auto-generated "update_<table>_updated_at" trigger before a data migration that wants
   * to set updated_at to a specific value (backfill from another column). Re-enable after.
   */
  async toggleTrigger(schema: string, tableName: string, triggerName: string, enabled: boolean): Promise<string> {
    validateSchema(schema);
    validateIdentifier(tableName, 'table name');
    validateIdentifier(triggerName, 'trigger name');
    const sql = `ALTER TABLE "${schema}"."${tableName}" ${enabled ? 'ENABLE' : 'DISABLE'} TRIGGER "${triggerName}"`;
    await this.db.raw(sql);
    return sql;
  }
  ...
  async truncateTable(
    ...
    // Refuse to truncate system-managed tables (same guard as dropTable).
    try {
      const comment = await this.db.raw(
      ...
      if (comment.rows[0]?.comment?.startsWith('system:')) {
        throw new AppError(403, `Table "${tableName}" is system-managed (plugin: ${comment.rows[0].comment.replace('system:', '')}). Disable the plugin to clear it.`);
      }
    } catch (e) {
    ...
    if (!exists.rows.length) {
      throw new AppError(404, `Table "${schema}"."${tableName}" not found (or it is a view/materialized view, which TRUNCATE cannot target).`);
    }

    // Hypertables: TimescaleDB makes TRUNCATE drop all chunks atomically.
    // We report the chunk count for visibility — there is no separate "truncate chunks" verb.
    let hypertableChunks = 0;
    try {
      const r: any = await this.db.raw(
        `SELECT count(*)::int AS n FROM timescaledb_information.chunks
         WHERE hypertable_schema = ? AND hypertable_name = ?`,
        [schema, tableName],
      );
      hypertableChunks = Number(r.rows?.[0]?.n ?? 0);
    } catch {}

    const parts = [`TRUNCATE TABLE "${schema}"."${tableName}"`];
    if (opts.restart_identity) parts.push('RESTART IDENTITY');
    if (opts.cascade) parts.push('CASCADE');
    await this.db.raw(parts.join(' '));

    return {
      table: `${schema}.${tableName}`,
      truncated: true,
      cascade: !!opts.cascade,
      restart_identity: !!opts.restart_identity,
      hypertable_chunks_dropped: hypertableChunks > 0 ? hypertableChunks : null,
    };
  }
  ...
  async addIndex(schema: string, tableName: string, idx: IndexDef) {
    ...
    // Validate / normalize ops_class — only known safe classes allowed
    const ALLOWED_OPS_CLASSES = new Set([
      'gin_trgm_ops', 'gist_trgm_ops',                    // pg_trgm (substring / similarity)
      'jsonb_ops', 'jsonb_path_ops',                      // jsonb GIN
      'inet_ops',                                         // network range lookups
      'array_ops',                                        // text[], int[] GIN
      'tsvector_ops',                                     // full-text search GIN/GIST
      'text_pattern_ops', 'varchar_pattern_ops',          // LIKE 'prefix%' btree
      'int4_ops', 'int8_ops', 'text_ops',                 // explicit default overrides
    ]);
    const opsClassOf = (idx: number): string | null => {
      if (!idx0.ops_class) return null;
      const v = Array.isArray(idx0.ops_class) ? idx0.ops_class[idx] : idx0.ops_class;
      if (!v) return null;
      if (!ALLOWED_OPS_CLASSES.has(v)) {
        throw new AppError(400, `Unknown ops_class "${v}". Allowed: ${[...ALLOWED_OPS_CLASSES].join(', ')}`);
      }
      return v;
    };
    const idx0 = idx; // closure alias for the helper

    let target: string;
    if (hasExprs) {
      // Expression form: if ops_class is provided, apply it; for gin/gist auto-install pg_trgm
      // when the user is clearly going for text search and ops_class is gin_trgm_ops / gist_trgm_ops.
      if (idx.type === 'gin' || idx.type === 'gist') {
        await this.db.raw('CREATE EXTENSION IF NOT EXISTS pg_trgm').catch(() => {});
      }
      target = idx.expressions!.map((e, i) => {
        const oc = opsClassOf(i);
        return oc ? `(${e}) ${oc}` : `(${e})`;
      }).join(', ');
      ...
      const defaultOps = idx.type === 'gin' ? 'gin_trgm_ops' : 'gist_trgm_ops';
      target = idx.columns!.map((c, i) => {
        const explicit = opsClassOf(i);
        if (explicit) return `"${c}" ${explicit}`;
        const dt = typeMap.get(c);
        return textTypes.has(dt) ? `"${c}" ${defaultOps}` : `"${c}"`;
      }).join(', ');
    } else {
      target = idx.columns!.map((c, i) => {
        const oc = opsClassOf(i);
