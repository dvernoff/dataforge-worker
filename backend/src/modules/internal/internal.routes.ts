...
export async function internalRoutes(app: FastifyInstance) {
  ...
  app.post('/quota/invalidate', async (request) => {
    const body = z.object({ project_id: z.string().uuid() }).parse(request.body);
    const invalidate = (app as any).invalidateQuotaCache as ((projectId: string) => void) | undefined;
    if (invalidate) {
      invalidate(body.project_id);
    } else {
      await app.redis.del(`project_quotas:${body.project_id}`);
    }
    return { success: true };
  });
