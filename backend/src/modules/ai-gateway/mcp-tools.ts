...
export const MCP_TOOLS: McpToolDef[] = [
  ...
  },
  {
    ...
    description: 'Create an API endpoint (table-based CRUD or custom SQL). Table endpoints auto-emit realtime events on every mutation. custom_sql endpoints do NOT emit automatically — add df_emit() calls inside your CTE (see get_websocket_info for examples) if real-time subscribers should see the changes.',
    inputSchema: {
      type: 'object',
      properties: {
        method: { type: 'string', enum: ['GET', 'POST', 'PUT', 'PATCH', 'DELETE'] },
        ...
        source_config: { type: 'object', description: 'For table: {table, operation}. For custom_sql: {query, params?}. Use :param_name casts inside query (e.g. :user_id::uuid). For realtime, include a MATERIALIZED df_emit CTE — see get_websocket_info.' },
  ...
  },
  {
    ...
    description: 'Execute a write SQL statement (INSERT, UPDATE, DELETE, MERGE). DDL (DROP/TRUNCATE/ALTER/CREATE/GRANT/REVOKE) is blocked — use schema tools instead. Requires confirm_write=true as an explicit guard. Supports {{name}} placeholders bound from "params". Realtime: by itself does NOT emit WS events — wrap your mutation in a CTE and call public.df_emit(table, action, pk) to broadcast (see get_websocket_info for the exact pattern). df_emit fires only on COMMIT, so it is transactional with your write.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  // ========== WebSocket ==========
  {
    ...
    description: 'Get WebSocket connection details for this project: URL, auth header, supported channels, server event shapes, limits, AND the df_emit() SQL bridge usage (how to emit realtime events from custom_sql CTEs, cron, execute_sql_mutation). Returns ready-to-paste multi-table CTE examples. Call this first when planning realtime for hot-path endpoints.',
    inputSchema: { type: 'object', properties: {}, required: [] },
  },
  {
    name: 'get_websocket_stats',
    description: 'Runtime WebSocket + pg_notify bridge stats for this project. Returns: connectedClients, messagesSent/Received, plus pg_notify {status, events_received_total, events_delivered_total, events_dropped_total, reconnects, last_event_at, per_table{table: count}}. Use after deploying df_emit() calls to verify events are arriving; investigate "dropped" count if events do not reach subscribers.',
    inputSchema: { type: 'object', properties: {}, required: [] },
  },
];
