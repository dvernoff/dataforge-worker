...
export const MCP_TOOLS: McpToolDef[] = [
  ...
  },
  {
    ...
    description: 'Execute a write SQL statement (INSERT, UPDATE, DELETE, MERGE). DDL (DROP/TRUNCATE/ALTER/CREATE/GRANT/REVOKE) is blocked — use schema tools instead. Requires confirm_write=true as an explicit guard. Supports {{name}} placeholders bound from "params". Realtime: by itself does NOT emit WS events — wrap your mutation in a CTE and call public.df_emit(table, action, pk) to broadcast. CRITICAL: the emit CTE MUST be referenced in the outer SELECT (e.g. "SELECT COUNT(*) FROM emit" or "FROM ins, emit") — AS MATERIALIZED alone is NOT enough, PostgreSQL still prunes unreferenced CTEs and df_emit silently never fires. See get_websocket_info for verified patterns. df_emit fires only on COMMIT, so it is transactional with your write.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: [
      ...
      'GOTCHAS:',
      '- The view is created WITH NO DATA. First reads return zero rows until the refresh policy fires (or you call CALL refresh_continuous_aggregate(view, NULL, NULL) once via execute_sql_mutation to backfill).',
    ...
    inputSchema: {
      ...
      properties: {
        ...
        source_table: { type: 'string', description: 'Must already be a hypertable (see create_hypertable).' },
