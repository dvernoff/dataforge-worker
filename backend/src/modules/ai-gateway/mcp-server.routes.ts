      ...
      try {
        ...
        switch (toolName) {
          ...
          case 'list_endpoints':
            ...
            break;
          case 'get_endpoint': {
            if (!args.id_or_path) throw new Error('Missing "id_or_path" (endpoint UUID or path like "/players/sync")');
            result = await service.getEndpoint(projectId, args.id_or_path as string, args.method as string | undefined);
            if (result === null) result = { message: `No endpoint found matching "${args.id_or_path}"${args.method ? ' ' + (args.method as string).toUpperCase() : ''}. Use list_endpoints or search_endpoints first.` };
            break;
          }
          case 'create_table': {
          ...
          case 'drop_table':
            ...
            break;
          case 'truncate_table': {
            if (!args.table_name) throw new Error('Missing "table_name"');
            result = await service.truncateTable(dbSchema, args.table_name as string, {
          ...
          case 'add_index':
            result = await service.addIndex(dbSchema, args.table_name as string, {
              ...
              ops_class: args.ops_class as string | string[] | undefined,
            });
            break;
          case 'drop_index':
          ...
          case 'update_endpoint':
            ...
            break;
          case 'bulk_update_endpoint': {
            const updates = args.updates as Array<{ endpoint_id: string } & Record<string, unknown>> | undefined;
            if (!Array.isArray(updates)) throw new Error('Missing "updates" array');
            result = await service.bulkUpdateEndpoint(projectId, updates);
          }
            break;
...
case 'create_materialized_view': {
          ...
          case 'list_continuous_aggregates':
            result = await service.listContinuousAggregates(dbSchema);
            break;
          case 'list_timescaledb_jobs':
            result = await service.listTimescaleJobs(dbSchema);
            break;
          case 'list_retention_policies':
            result = await service.listRetentionPolicies(dbSchema);
            break;
          case 'list_compression_policies':
            result = await service.listCompressionPolicies(dbSchema);
            break;
          case 'refresh_continuous_aggregate': {
            if (!args.view_name) throw new Error('Missing "view_name"');
            result = await service.refreshContinuousAggregate(dbSchema, args as Parameters<AiGatewayService['refreshContinuousAggregate']>[1]);
            break;
          }
          case 'remove_retention_policy': {
            if (!args.table) throw new Error('Missing "table"');
            result = await service.removeRetentionPolicy(dbSchema, args.table as string);
            break;
          }
          case 'remove_compression_policy': {
            if (!args.table) throw new Error('Missing "table"');
            result = await service.removeCompressionPolicy(dbSchema, args.table as string);
            break;
          }
          case 'remove_continuous_aggregate_policy': {
            if (!args.view_name) throw new Error('Missing "view_name"');
            result = await service.removeContinuousAggregatePolicy(dbSchema, args.view_name as string);
            break;
          }
          case 'update_retention_policy': {
            if (!args.table || !args.drop_after) throw new Error('Missing "table" or "drop_after"');
            result = await service.updateRetentionPolicy(dbSchema, args as Parameters<AiGatewayService['updateRetentionPolicy']>[1]);
            break;
          }
          case 'update_compression_policy': {
            if (!args.table || !args.compress_after) throw new Error('Missing "table" or "compress_after"');
            result = await service.updateCompressionPolicy(dbSchema, args as Parameters<AiGatewayService['updateCompressionPolicy']>[1]);
            break;
          }
          case 'update_continuous_aggregate_policy': {
            if (!args.view_name || !args.start_offset || !args.end_offset || !args.schedule_interval) {
              throw new Error('Missing required field (view_name, start_offset, end_offset, schedule_interval)');
            }
            result = await service.updateContinuousAggregatePolicy(dbSchema, args as Parameters<AiGatewayService['updateContinuousAggregatePolicy']>[1]);
            break;
          }
          case 'drop_continuous_aggregate': {
            if (!args.view_name) throw new Error('Missing "view_name"');
            result = await service.dropContinuousAggregate(dbSchema, args.view_name as string, !!args.cascade);
            break;
          }
          case 'drop_hypertable': {
            if (!args.table) throw new Error('Missing "table"');
            result = await service.dropHypertable(dbSchema, args.table as string, !!args.cascade);
            break;
          }
          case 'drop_chunks': {
            if (!args.hypertable || !args.older_than) throw new Error('Missing "hypertable" or "older_than"');
            result = await service.dropChunks(dbSchema, args as Parameters<AiGatewayService['dropChunks']>[1]);
            break;
          }
          case 'alter_timescaledb_job': {
            if (!args.job_id) throw new Error('Missing "job_id"');
            result = await service.alterTimescaleJob(dbSchema, args as Parameters<AiGatewayService['alterTimescaleJob']>[1]);
            break;
          }
          case 'compress_chunk': {
            if (!args.chunk_name) throw new Error('Missing "chunk_name"');
            result = await service.compressChunk(dbSchema, args as Parameters<AiGatewayService['compressChunk']>[1]);
            break;
          }
          case 'decompress_chunk': {
            if (!args.chunk_name) throw new Error('Missing "chunk_name"');
            result = await service.decompressChunk(dbSchema, args as Parameters<AiGatewayService['decompressChunk']>[1]);
            break;
          }
          case 'recompress_chunk': {
            if (!args.chunk_name) throw new Error('Missing "chunk_name"');
            result = await service.recompressChunk(dbSchema, args as Parameters<AiGatewayService['recompressChunk']>[1]);
            break;
          }
          case 'run_timescaledb_job': {
            if (!args.job_id) throw new Error('Missing "job_id"');
            result = await service.runTimescaleJob(dbSchema, args as Parameters<AiGatewayService['runTimescaleJob']>[1]);
            break;
          }
          case 'set_chunk_time_interval': {
            if (!args.hypertable || !args.new_interval) throw new Error('Missing "hypertable" or "new_interval"');
            result = await service.setChunkTimeInterval(dbSchema, args as Parameters<AiGatewayService['setChunkTimeInterval']>[1]);
            break;
          }
          ...
          // ===== Plugins =====
          case 'list_plugins':
            result = await service.listPlugins(projectId);
            break;
          case 'get_plugin': {
            if (!args.plugin_id) throw new Error('Missing "plugin_id"');
            result = await service.getPluginInfo(projectId, args.plugin_id as string);
            break;
          }
          case 'enable_plugin': {
            if (!args.plugin_id) throw new Error('Missing "plugin_id"');
            result = await service.enablePluginForProject(projectId, args.plugin_id as string, (args.settings as Record<string, unknown>) ?? {});
            break;
          }
          case 'disable_plugin': {
            if (!args.plugin_id) throw new Error('Missing "plugin_id"');
            result = await service.disablePluginForProject(projectId, args.plugin_id as string);
            break;
          }
          case 'update_plugin_settings': {
            if (!args.plugin_id || !args.settings) throw new Error('Missing "plugin_id" or "settings"');
            result = await service.updatePluginSettingsForProject(projectId, args.plugin_id as string, args.settings as Record<string, unknown>);
            break;
          }

          // ===== Webhooks =====
          case 'list_webhooks':
            result = await service.listWebhooks(projectId);
            break;
          case 'get_webhook': {
            if (!args.webhook_id) throw new Error('Missing "webhook_id"');
            result = await service.getWebhook(projectId, args.webhook_id as string);
            break;
          }
          case 'create_webhook': {
            if (!Array.isArray(args.table_names) || !Array.isArray(args.events) || !args.url) {
              throw new Error('Missing required: table_names (array), events (array), url');
            }
            result = await service.createWebhook(projectId, null, args as Parameters<AiGatewayService['createWebhook']>[2]);
            break;
          }
          case 'update_webhook': {
            if (!args.webhook_id) throw new Error('Missing "webhook_id"');
            const { webhook_id, ...rest } = args;
            result = await service.updateWebhook(projectId, webhook_id as string, rest);
            break;
          }
          case 'delete_webhook': {
            if (!args.webhook_id) throw new Error('Missing "webhook_id"');
            result = await service.deleteWebhook(projectId, args.webhook_id as string);
            break;
          }
          case 'get_webhook_logs': {
            if (!args.webhook_id) throw new Error('Missing "webhook_id"');
            result = await service.getWebhookLogs(projectId, args.webhook_id as string, args.limit as number | undefined);
            break;
          }
          ...
          case 'optimize_table': {
            if (!args.table_name) throw new Error('Missing "table_name"');
            result = await service.optimizeTable(dbSchema, args.table_name as string);
            break;
          }
          ...
          case 'list_triggers': {
            if (!args.table_name) throw new Error('Missing "table_name"');
            result = await service.listTriggers(dbSchema, args.table_name as string);
            break;
          }
          case 'toggle_trigger': {
            if (!args.table_name) throw new Error('Missing "table_name"');
            if (typeof args.enabled !== 'boolean') throw new Error('Missing "enabled" (boolean)');
            result = await service.toggleTrigger(
              dbSchema,
              args.table_name as string,
              args.trigger_name as string | undefined,
          ...
          case 'get_websocket_info': {
            const slugRow = await app.db('projects').where({ id: projectId }).first();
            const hostHeader = (request.headers['x-forwarded-host'] as string) ?? request.headers.host ?? 'your-dataforge-host';
            result = service.getWebsocketInfo(slugRow?.slug ?? '', hostHeader);
            break;
          }
          case 'get_websocket_stats':
            result = service.getWebsocketStats(projectId);
            break;
