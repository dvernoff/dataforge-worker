...
CAPABILITIES:
- Schema: create/alter/drop tables, indexes (btree/hash/gin/gist/brin, partial, expression, covering), foreign keys, materialized views
...
COLUMN TYPES
==========================================================
text, integer, bigint, float, decimal, boolean, date, timestamp, timestamptz, uuid, json, jsonb, text[], integer[], serial, bigserial

==========================================================
CREATE TABLE (use create_table)
==========================================================
- add_uuid_pk (default true) — adds "id" UUID PRIMARY KEY DEFAULT gen_random_uuid()
- add_timestamps (default true) — adds created_at / updated_at TIMESTAMPTZ with update trigger
...
- {action: "alter", name, type, nullable} — change type or nullability
...
- hash — equality only
- gin — JSONB @>, array &&, full-text, pg_trgm for LIKE
- gist — geometric, ranges, ts_vector
