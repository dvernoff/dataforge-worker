  ...
  getWebsocketInfo(projectSlug: string, host = 'your-dataforge-host') {
    return {
      ...
      event_sources: {
        ...
        example_custom_sql: "WITH upserted AS (INSERT INTO players ... ON CONFLICT DO UPDATE RETURNING *), _emit AS MATERIALIZED (SELECT df_emit('players','UPSERT',id::text, to_jsonb(u)) FROM upserted u) SELECT u.* FROM upserted u, _emit;",
        example_multi_table_cte: "WITH p AS (INSERT INTO players ... RETURNING *), i AS (INSERT INTO player_ips ... RETURNING *), ep AS MATERIALIZED (SELECT df_emit('players','UPSERT',id::text) FROM p), ei AS MATERIALIZED (SELECT df_emit('player_ips','INSERT',id::text) FROM i) SELECT 1 FROM ep, ei;",
        planner_warning: "A SELECT-only CTE (read-only) MUST be either AS MATERIALIZED, or referenced in the outer SELECT (CROSS JOIN / FROM …, emit_cte). Otherwise PostgreSQL may skip it and the event will not fire.",
      },
...
CRITICAL — PostgreSQL CTE evaluation semantics:
  A WITH clause containing only SELECT (no INSERT/UPDATE/DELETE) is NOT guaranteed to execute
  unless the outer query references it. A "SELECT df_emit(...)" CTE that is never joined into
  the final SELECT may be skipped by the planner and the event will never fire.
  Two reliable fixes, either one works:

  (a) Mark the emit CTE as MATERIALIZED (PG 12+) — forces evaluation:
      emit_p AS MATERIALIZED (SELECT df_emit('players','UPSERT',id::text) FROM p)

  (b) Reference the emit CTE in the final SELECT (CROSS JOIN or similar):
      SELECT p.* FROM p, emit_p;
...
Multi-table atomic CTE pattern (hot path sync: players + player_ips + player_nicks):
  WITH p AS (INSERT INTO players ... RETURNING *),
       ...
       ep AS MATERIALIZED (SELECT df_emit('players',      'UPSERT', id::text) FROM p),
       ei AS MATERIALIZED (SELECT df_emit('player_ips',   'INSERT', id::text) FROM i),
       en AS MATERIALIZED (SELECT df_emit('player_nicks', 'INSERT', id::text) FROM n)
