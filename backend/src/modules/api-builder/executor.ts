...
export class Executor {
  ...
  private async executeSQL(
    ...
    const config = endpoint.source_config;
    // Accept both {query} (canonical, per MCP schema) and {sql} (commonly written by hand and
    // shown in df_emit examples). Pick whichever is a non-empty string.
    const sql = (typeof config.query === 'string' && config.query.trim())
      ? (config.query as string)
      : (typeof config.sql === 'string' ? (config.sql as string) : '');
    if (!sql) {
      throw new AppError(400, 'custom_sql endpoint has empty source_config.query (or source_config.sql). Provide the SQL body.');
    }
