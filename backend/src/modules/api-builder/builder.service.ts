...
export class BuilderService {
  ...
  async update(id: string, projectId: string, input: Partial<CreateEndpointInput>, db: Knex | Knex.Transaction = this.db) {
    ...
    if (input.required_scopes !== undefined) updateData.required_scopes = JSON.stringify(input.required_scopes);
      ...
      const existing = await db('api_endpoints').where({ id, project_id: projectId }).first();
    ...
    const [endpoint] = await db('api_endpoints')
      .where({ id, project_id: projectId })
      .update(updateData)
      .returning('*');
  ...
  async bulkUpdate(
    projectId: string,
    updates: Array<{ endpoint_id: string } & Partial<CreateEndpointInput>>,
  ) {
    if (!Array.isArray(updates) || updates.length === 0) {
      throw new AppError(400, 'updates must be a non-empty array');
    }
    if (updates.length > 100) {
      throw new AppError(400, `Bulk update size limit is 100 endpoints per call (got ${updates.length}). Split into smaller batches.`);
    }
    type Result = { endpoint_id: string; status: 'ok' | 'error'; error?: string; endpoint?: Record<string, unknown> };
    const results: Result[] = [];
    // Manual transaction control: collect per-row outcomes, decide commit/rollback at the end.
    // All-or-nothing on the wire (rollback if ANY row fails) plus per-row diagnostics so the
    // caller can see exactly which entry blew up — without losing the others to a generic error.
    const trx = await this.db.transaction();
    try {
      for (const u of updates) {
        const { endpoint_id, ...patch } = u;
        if (!endpoint_id) {
          results.push({ endpoint_id: '', status: 'error', error: 'Missing endpoint_id' });
          continue;
        }
        try {
          const endpoint = await this.update(endpoint_id, projectId, patch as Partial<CreateEndpointInput>, trx);
          results.push({ endpoint_id, status: 'ok', endpoint });
        } catch (err) {
          results.push({ endpoint_id, status: 'error', error: (err as Error).message });
        }
      }
