...
export async function settingsRoutes(app: FastifyInstance) {
  ...
  // ---------- Ensure AI Usage Log table ----------
  async function ensureAiUsageLogTable() {
    const exists = await app.db.schema.hasTable('ai_usage_log');
    if (!exists) {
      await app.db.schema.createTable('ai_usage_log', (t) => {
        t.uuid('id').primary().defaultTo(app.db.raw('gen_random_uuid()'));
  ...
  // Ensure AI quota columns exist in user_quotas and default_quotas tables
  async function ensureAiQuotaColumns() {
    for (const tableName of ['user_quotas', 'default_quotas']) {
      const hasTable = await app.db.schema.hasTable(tableName);
      if (!hasTable) continue;
      const hasAiReqs = await app.db.schema.hasColumn(tableName, 'max_ai_requests_per_day');
      if (!hasAiReqs) {
        await app.db.schema.alterTable(tableName, (t) => {
  ...
  await ensureAiQuotaColumns();

  // GET /api/system/ai/usage — get AI usage for current user today
  app.get('/ai/usage', {
  ...
  // GET /api/system/ai/budget — get monthly AI spend, budget, alert threshold (superadmin)
  app.get('/ai/budget', {
    preHandler: [authMiddleware],
  }, async (request, reply) => {
    ...
    // Calculate cost using: input * $3/MTok + output * $15/MTok
    const now = new Date();
    const monthStart = new Date(now.getFullYear(), now.getMonth(), 1).toISOString();

    const monthlyUsage = await app.db('ai_usage_log')
      .where('created_at', '>=', monthStart)
      .select(
        app.db.raw('COUNT(*)::int as total_requests'),
        app.db.raw('COALESCE(SUM(input_tokens), 0)::bigint as total_input_tokens'),
        app.db.raw('COALESCE(SUM(output_tokens), 0)::bigint as total_output_tokens'),
        app.db.raw('COALESCE(SUM(cost_usd), 0)::numeric(10,4) as total_cost_usd')
      )
      .first();

    // If cost_usd was not calculated at insert time, compute from tokens
    let totalCost = parseFloat(monthlyUsage?.total_cost_usd ?? '0');
    if (totalCost === 0 && (monthlyUsage?.total_input_tokens > 0 || monthlyUsage?.total_output_tokens > 0)) {
      const inputTokens = parseInt(monthlyUsage.total_input_tokens, 10) || 0;
      const outputTokens = parseInt(monthlyUsage.total_output_tokens, 10) || 0;
      totalCost = (inputTokens * 3 / 1_000_000) + (outputTokens * 15 / 1_000_000);
    }

    const budgetLimit = parseFloat(await getSetting('ai_monthly_budget_usd') ?? '0');
    const alertThreshold = parseFloat(await getSetting('ai_budget_alert_pct') ?? '80');

    return {
      month: `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`,
      total_requests: monthlyUsage?.total_requests ?? 0,
      total_input_tokens: parseInt(monthlyUsage?.total_input_tokens ?? '0', 10),
      total_output_tokens: parseInt(monthlyUsage?.total_output_tokens ?? '0', 10),
  ...
  // PUT /api/system/ai/budget — update budget settings (superadmin)
  app.put('/ai/budget', {
    preHandler: [authMiddleware],
  }, async (request, reply) => {
    if (!request.user.is_superadmin) {
    ...
    if (body.budget_limit_usd !== undefined) {
      await setSetting('ai_monthly_budget_usd', String(body.budget_limit_usd));
    }
    if (body.alert_threshold_pct !== undefined) {
      await setSetting('ai_budget_alert_pct', String(body.alert_threshold_pct));
    }
